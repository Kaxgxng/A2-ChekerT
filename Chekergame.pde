...
void mousePressed() {
    ...
    // Clicked on a piece
    if (selectedPiece == null && (board[row][col] != 0 && board[row][col] != -1)) {
        // Select a piece to move (only the current player's pieces)
        if ((isBlackTurn && (board[row][col] == 2 || board[row][col] == 4)) ||
            (!isBlackTurn && (board[row][col] == 1 || board[row][col] == 3))) {

            if (anyCaptureAvailable(isBlackTurn ? 2 : 1)) {
                if (isCaptureMoveAvailable(row, col)) {
                    selectedPiece = new PVector(col, row); // Select piece that can capture
                }
            } else {
        ...
        // Allow player to deselect the currently selected piece by clicking it again
        if (selectedPiece.x == col && selectedPiece.y == row) {
            ...
            if (isValidMove(oldRow, oldCol, row, col) || isCaptureMove(oldRow, oldCol, row, col)) {
                ...
                // Optionally, you can deselect the piece on invalid move
                // selectedPiece = null;
            }
        }
    }
...
boolean isValidMove(int oldRow, int oldCol, int newRow, int newCol) {
    ...
    if (piece == 1 || piece == 2) { 
        ...
        // Ensure that the piece only moves diagonally forward
    ...
    // Kings can move multiple squares diagonally
        ...
        if (abs(newRow - oldRow) == abs(newCol - oldCol)) {
            // Check if King can move in a straight diagonal path
...
boolean anyCaptureAvailable(int pieceType) {
    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
            if (board[row][col] == pieceType || board[row][col] == pieceType + 2) { // Check for regular piece or king
                if (isCaptureMoveAvailable(row, col)) {
                    println("Capture available for piece at (" + row + ", " + col + ")");
                    return true; // Found a piece that can capture
                }
            }
        }
    }
